...
setup_asd() {
    ...
    #   - "ASD_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "ASD_" variables is documented below.
    #
    ...
    export ASD_BASE="${this_dir}"
    export CF_BASE="${this_dir}/modules/columnflow"
    export CF_REPO_BASE="${ASD_BASE}"
    ...
    if [ "${CF_REMOTE_JOB}" != "1" ]; then
    	cf_setup_interactive_body() {
            ...
            query CF_DATA "Local data directory" "\$ASD_BASE/data" "./data"
            query CF_STORE_NAME "Relative path used in store paths (see next queries)" "asd_store"
		...
		export_and_save CF_SCHEDULER_PORT "8082"
            ...
            query ASD_BUNDLE_CMSSW "Install and bundle CMSSW sandboxes for job submission?" "True"
	}
	cf_setup_interactive "${CF_SETUP_NAME}" "${ASD_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${ASD_BASE}/bin:${PATH}"
    export PYTHONPATH="${ASD_BASE}:${ASD_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -d "${ASD_BASE}/.git" ]; then
        for m in $( ls -1q "${ASD_BASE}/modules" ); do
            cf_init_submodule "${ASD_BASE}/modules/${m}"
        done
    fi
    ...
    export LAW_HOME="${ASD_BASE}/.law"
    export LAW_CONFIG_FILE="${ASD_BASE}/law.cfg"
    export LAW_CONFIG_FILE="${ASD_BASE}/custom_law.cfg"
...
main() {
    ...
    if setup_asd "$@"; then
        cf_color green "asd2qwe analysis successfully setup"
        return "0"
    else
        local code="$?"
