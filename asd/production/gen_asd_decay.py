...
@producer
def gen_asd_decay_products(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    """
    Creates column 'gen_asd_decay', which includes the most relevant particles of a HH->bbWW(qqlnu) decay.
    All sub-fields correspond to individual GenParticles with fields pt, eta, phi, mass and pdgId.
    """

    if self.dataset_inst.is_data or not self.dataset_inst.x("is_asd", False):
        return events
    ...
    gen_asd_decay = ak.Array({
        gp: {f: np.float32(hhgen[gp][f]) for f in ["pt", "eta", "phi", "mass", "pdgId"]} for gp in hhgen.keys()
    })
    events = set_ak_column(events, "gen_asd_decay", gen_asd_decay)

    return events


@gen_asd_decay_products.init
def gen_asd_decay_products_init(self: Producer) -> None:
    """
    Ammends the set of used and produced columns of :py:class:`gen_asd_decay_products` in case
    a dataset including top decays is processed.
    """
    if getattr(self, "dataset_inst", None) and self.dataset_inst.x("is_asd", False):
        self.uses |= {"nGenPart", "GenPart.*"}
        self.produces |= {"gen_asd_decay"}
